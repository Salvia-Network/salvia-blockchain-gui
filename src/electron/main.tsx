...
import salviaEnvironment from '../util/salviaEnvironment';
import salviaConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!salviaEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    salviaConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!salviaConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !salviaConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (salviaConfig.manageDaemonLifetime()) {
        salviaEnvironment.startSalviaDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Salvia Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Salvia-Network/salvia-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Salvia-Network/salvia-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Salvia-Network/salvia-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Salvia-Network/salvia-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Salvia-Network/salvia-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/salvia_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/salvia_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Salvia Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Salvia' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Salvia Blockchain' }),
            click: () => {
              openAbout();
            },
          },
