...
const getSalviaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('salvia');
  // first see if we can get a salvia exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or salvia is on the path
    try {
      version = child_process
