...
export default function Block() {
  ...
  const poolReward = seed_to_salvia(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = seed_to_salvia(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const salviaFees = blockRecord.fees
    ? seed_to_salvia(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.salviaexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.salviaexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: salviaFees ? `${salviaFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Salvia blockchain
            </Trans>
          </Back>
        }
        action={
