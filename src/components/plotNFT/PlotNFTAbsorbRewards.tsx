...
import { seed_to_salvia, salvia_to_seed } from '../../util/salvia';
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const feeSeeds = salvia_to_seed(fee);

      if (walletId === undefined || !address) {
        return;
      }

      await dispatch(pwAbsorbRewards(walletId, feeSeeds));
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={seed_to_salvia(BigInt(balance))}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {wallet?.address}
