...
import { salvia_to_seed, colouredcoin_to_seed } from '../../util/salvia';
  ...
  function handleAdd() {
    ...
    const seed =
      wallets[wallet_id.value].type === COLOURED_COIN
        ? colouredcoin_to_seed(amount_input.value)
        : salvia_to_seed(amount_input.value);

    const trade =
      buy_or_sell.value === 1
        ? newBuy(seed, wallet_id.value)
        : newSell(seed, wallet_id.value);
