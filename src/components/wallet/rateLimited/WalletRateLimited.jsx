...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const salviaper_input = ip_parsed.limit;
    ...
    const salviaper_value = Number.parseInt(Number(salviaper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (salvia per interval):{' '}
                  {seed_to_salvia_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = salvia_to_seed(amount_input.value);
    const fee = salvia_to_seed(fee_input.value);
