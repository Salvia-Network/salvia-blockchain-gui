...
import { AlertDialog, Card, Flex } from '@salvia/core';
...
import {
  seed_to_colouredcoin_string,
  colouredcoin_to_seed,
} from '../../../util/salvia';
      ...
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {seed_to_colouredcoin_string(props.balance)} {cc_unit}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_salvia = seed_to_colouredcoin_string(balance_ptotal);
  const balance_pending_salvia = seed_to_colouredcoin_string(balance_pending);
  const balance_change_salvia = seed_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
    balancebox_3 +
    balance_ptotal_salvia +
    balancebox_unit +
    balancebox_hline +
    balancebox_4 +
    balancebox_row +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    const amount = colouredcoin_to_seed(amount_input.value);
    const fee = colouredcoin_to_seed(fee_input.value);

    if (address.includes('salvia_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
