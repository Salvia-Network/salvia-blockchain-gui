...
import { AlertDialog } from '@salvia/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      salviaper_input.value === '' ||
      Number(salviaper_input.value) === 0 ||
      !Number(salviaper_input.value) ||
      isNaN(Number(salviaper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const salviaper = salvia_to_seed(salviaper_input.value);
    const salviaper_value = Number.parseInt(Number(salviaper));
    ...
    // var fee = salvia_to_seed(fee_input.value);
